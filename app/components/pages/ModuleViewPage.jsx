...
function InfoCard({
  ...
  return (
    <div
      className={`bg-white border ${borderColor} rounded-2xl p-5 shadow-sm flex items-center gap-4`}
    >
      <div
    ...
    <EditorLayout>
      <div className="space-y-6">
        ...
        <div className="flex items-center gap-5">
          ...
          <div
            className={`w-16 h-16 rounded-2xl bg-linear-to-br ${meta.color} flex items-center justify-center shrink-0 shadow-sm`}
          >
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-3 flex-wrap">
              <h2 className="text-2xl font-black text-slate-950">
                {htmlToText(mod.title)}
              </h2>
              <span
                className={`px-2.5 py-0.5 text-[11px] font-black rounded-full border capitalize ${meta.light} ${meta.text} ${meta.border}`}
              >
                {meta.label}
              </span>
            </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          ...
          <InfoCard
            icon={Clock}
            label="Time Limit"
            value={mod.time_limit_sec ? `${mod.time_limit_sec}s` : "—"}
          ...
          <InfoCard
            icon={BookOpen}
            label="Total Marks"
            value={mod.total_marks > 0 ? mod.total_marks : "—"}
        ...
        <div className="bg-white border border-orange-500/20 rounded-2xl p-6 shadow-sm">
          ...
          <div className="grid grid-cols-2 gap-5 text-sm">
            ...
            <div>
              ...
              <p className={`font-bold capitalize ${meta.text}`}>
                {meta.label}
              </p>
            </div>
            {mod.description && (
            ...
            {mod.video && (
              <>
                ...
                <div>
                  ...
                  <p className="font-bold text-slate-800">
                    {mod.video.duration_sec
                      ? `${mod.video.duration_sec}s`
                      : "—"}
                  </p>
                </div>
              </>
          ...
          <div className="bg-white border border-orange-500/20 rounded-2xl shadow-sm overflow-hidden">
            <div className="px-6 py-4 border-b border-orange-500/10 flex items-center justify-between">
              ...
              <span className="text-xs font-bold text-orange-600">
                {mod.words.length} words
              </span>
            </div>
            <div className="divide-y divide-orange-500/5">
              {mod.words.map((w, i) => (
                <div key={i} className="px-6 py-3 flex items-start gap-4">
                  <div className="w-8 h-8 rounded-lg bg-orange-100 flex items-center justify-center font-black text-orange-700 text-sm shrink-0">
                    {i + 1}
                  </div>
                  <div>
                    ...
                    <div className="mt-1.5 space-y-0.5">
                      {q.options.map((o, oi) => {
                        ...
                        return (
                          <p
                            key={oi}
                            className={`text-xs px-2 py-1 rounded ${correctByLetter || correctByText ? "bg-green-100 text-green-700 font-bold" : "text-slate-500"}`}
                          >
