  ...
  return (
    <div className="border-t border-slate-200 pt-3">
      ...
      {questions.map((q, i) => (
        <div
          key={i}
          className="bg-slate-50 border border-slate-200 rounded-xl p-3 mb-2 space-y-2"
        >
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <input
              ...
              value={q.explanation || ""}
              onChange={(e) => updQ(i, "explanation", e.target.value)}
            />
          </div>
          {withNegativeMarks && (
            <div>
          ...
          {withParagraphRef && (
            <div>
              <label className="block text-[10px] font-semibold text-slate-500 mb-1">
                Paragraph Ref (paragraph number this question refers to)
              </label>
              <input
                type="number"
                min={1}
...
function WordsSection({ words, setWords, withDetails = false }) {
  ...
  const updW = (i, k, v) =>
    setWords((p) => p.map((w, idx) => (idx === i ? { ...w, [k]: v } : w)));

  return (
    <div className="border-t border-slate-200 pt-3">
      <div className="flex justify-between mb-2">
        <p className="text-xs font-bold text-slate-600">
          Words <span className="font-normal text-slate-400">(optional)</span>
        </p>
        <button
          type="button"
          onClick={addW}
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-2">
            <input
              ...
              placeholder="Word *"
              value={w.word}
              onChange={(e) => updW(i, "word", e.target.value)}
            />
            <input
              className={inp}
              placeholder="Meaning *"
              value={w.meaning}
              onChange={(e) => updW(i, "meaning", e.target.value)}
            />
          </div>
          {withDetails && (
            <div className="grid grid-cols-2 gap-2">
              ...
              <select
                className={`${inp} cursor-pointer`}
                value={w.part_of_speech || "noun"}
                onChange={(e) => updW(i, "part_of_speech", e.target.value)}
              >
                {POS_OPTIONS.map((p) => (
          ...
          <div className="grid grid-cols-2 gap-2">
            <input
              ...
              value={w.example || ""}
              onChange={(e) => updW(i, "example", e.target.value)}
            />
            {withDetails && (
              <select
                className={`${inp} cursor-pointer`}
                value={w.difficulty || "easy"}
                onChange={(e) => updW(i, "difficulty", e.target.value)}
              >
                {DIFF_OPTIONS.map((d) => (
...
function TextForm({ topicId, subtopicId, onDone, onCancel }) {
  ...
  const submit = async (e) => {
    ...
    if (!f.body.trim()) {
      Swal.fire({ icon: "warning", title: "Content body is required" });
      return;
    }
  ...
  return (
    <form onSubmit={submit} className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          ...
          <input
            ...
            className={inp}
            placeholder="e.g. 3"
            value={f.read_time_min}
            onChange={set("read_time_min")}
          />
        </div>
      </div>
      ...
      <div>
        ...
        <input
          className={inp}
          placeholder="e.g. Oxford English Grammar (optional)"
          value={f.source}
          onChange={set("source")}
        />
      </div>
...
function VideoForm({ topicId, subtopicId, onDone, onCancel }) {
  ...
  const submit = async (e) => {
    ...
    if (!title || !file) {
      Swal.fire({
        icon: "warning",
        title: "Title and video file are required",
      });
      return;
    }
    setSaving(true);
    try {
      ...
      fd.append("sub_topic_id", subtopicId);
      ...
      const validQs = questions.filter(
        (q) => q.question_text && q.correct_answer,
      );
    ...
    <form onSubmit={submit} className="space-y-3">
      ...
      <div>
        ...
        <input
          ...
          className="w-full text-sm text-slate-500 file:mr-3 file:py-2 file:px-4 file:rounded-xl file:border-0 file:font-semibold file:bg-orange-50 file:text-orange-700 hover:file:bg-orange-100 border border-slate-300 rounded-xl px-3 py-2"
        />
      </div>
...
function AudioForm({ topicId, subtopicId, onDone, onCancel }) {
  ...
  const [replayLimit, setReplayLimit] = useState(3);
  const [file, setFile] = useState(null);
  const [words, setWords] = useState([]);
  const [questions, setQuestions] = useState([]);
  const [saving, setSaving] = useState(false);
  ...
  const submit = async (e) => {
    ...
    if (!title || !file) {
      Swal.fire({
        icon: "warning",
        title: "Title and audio file are required",
      });
      return;
    }
    setSaving(true);
    try {
      ...
      fd.append("title", title);
      fd.append("order", order);
      fd.append("allow_replay", allowReplay);
      fd.append("replay_limit", replayLimit);
      fd.append(
        "audio",
        JSON.stringify({
          type: audioType,
  ...
  return (
    <form onSubmit={submit} className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          ...
          <select
            ...
            value={f.difficulty}
            onChange={(e) =>
              setF((p) => ({ ...p, difficulty: e.target.value }))
            }
          >
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <input
            ...
            value={f.max_attempts}
            onChange={(e) =>
              setF((p) => ({ ...p, max_attempts: e.target.value }))
            }
          />
        </div>
        <div className="flex flex-col gap-1.5 pt-1">
          {[
            ...
            <label
              key={k}
              className="flex items-center gap-2 cursor-pointer text-xs font-semibold text-slate-600"
            >
              <input
                type="checkbox"
                checked={f[k]}
      ...
      <QuestionsSection
        questions={questions}
        setQuestions={setQuestions}
        qtypes={Q_TYPES_EXERCISE}
        withNegativeMarks={true}
      />
      <FormBtns saving={saving} onCancel={onCancel} />
    </form>
  );
...
function VocabForm({ topicId, subtopicId, onDone, onCancel }) {
  ...
  const submit = async (e) => {
    e.preventDefault();
    if (!f.title || words.some((w) => !w.word || !w.meaning)) {
      Swal.fire({
        icon: "warning",
        title: "Title, word and meaning are required",
      });
      return;
    }
    setSaving(true);
  ...
  return (
    <div className="flex gap-3 pt-3 border-t border-slate-200">
      ...
      <button
        type="button"
        onClick={onCancel}
        className="px-5 py-2 border border-slate-300 text-slate-600 text-sm font-semibold rounded-xl hover:bg-slate-50 transition cursor-pointer"
      >
        Cancel
      </button>
    </div>
...
export default function ModuleTypePage({ type, addUrl }) {
  ...
  const onSubtopicChange = async (id) => {
    ...
    setLoadingMods(true);
    try {
      const r = await getModules(type, id);
      setModules(
        Array.isArray(r.data?.data || r.data) ? r.data?.data || r.data : [],
      );
    } catch {
