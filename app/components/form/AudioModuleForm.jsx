...
export default function AudioModuleForm({ onSubmit, onCancel, saving = false }) {
  ...
  const v = {
    ...
    replayLimit:  allowReplay && +replayLimit < 1,
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 w-full bg-white p-8 rounded-3xl border border-orange-500/20 shadow-xl">
      <h3 className="text-xl font-black text-[#3C1E0A] border-b border-orange-500/10 pb-4">Create Audio Module</h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="md:col-span-2">
          ...
          <input className={fc(s && v.title)} placeholder="Audio module title" value={title} onChange={e => setTitle(e.target.value)} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div>
          ...
          <div className="relative">
            <select className={`${inp} cursor-pointer pr-9 appearance-none`} value={audioType} onChange={e => setAudioType(e.target.value)}>
        ...
        <div>
          ...
          <div className="relative">
            <select className={`${inp} cursor-pointer pr-9 appearance-none`} value={speed} onChange={e => setSpeed(e.target.value)}>
        ...
        <div>
          ...
          <input type="number" min={1} className={fc(s && v.duration)} placeholder="e.g. 180 (optional)" value={durationSec} onChange={e => setDurationSec(e.target.value)} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className={!allowReplay ? "opacity-40 pointer-events-none" : ""}>
          <label className={lbl}>Replay Limit</label>
          <input type="number" min={1} className={fc(s && v.replayLimit)} value={replayLimit} disabled={!allowReplay} onChange={e => setReplayLimit(e.target.value)} />
          {err(s && v.replayLimit, "Replay limit must be at least 1")}
        </div>
      </div>

      <div>
        <label className={lbl}>Transcript <span className="text-gray-400 font-normal">(optional)</span></label>
        <textarea className={`${inp} resize-none`} rows={4} placeholder="Full transcript of the audio…" value={transcript} onChange={e => setTranscript(e.target.value)} />
      </div>
      <div>
        ...
        />
        {err(s && v.file, "Audio file is required")}
      </div>
      ...
      <div className="flex justify-end gap-4 pt-4 border-t border-orange-500/10">
        ...
        <Button type="submit" disabled={saving}>{saving ? "Creating…" : "Create Audio Module"}</Button>
      </div>
    </form>
  );
}
