...
const Q_TYPES      = ["mcq", "fill_blank", "match_meaning", "spell_word"];
...
export default function VocabularyModuleForm({ onSubmit, onCancel, saving = false }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 w-full bg-white p-8 rounded-3xl border border-orange-500/20 shadow-xl">
      ...
      {err(s && v.noWords, "At least one word is required")}
          ...
          <div key={i} className={`rounded-2xl p-5 space-y-4 border ${s && (wv[i]?.word || wv[i]?.meaning) ? "bg-red-50/40 border-red-200" : "bg-orange-50/40 border-orange-200"}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input className={fc(s && wv[i]?.meaning)} placeholder="Fluent or persuasive in speaking" value={w.meaning} onChange={e => updW(i, "meaning", e.target.value)} />
            ...
            <div>
              <label className={lbl}>Example Sentence <span className="text-gray-400 font-normal">(optional)</span></label>
      ...
      {/* ── Questions ────────────────────────────────────────────────────── */}
      <SectionDivider title="Practice Questions" />
      <div className="flex items-center justify-between">
        <p className="text-sm font-medium text-gray-700">
