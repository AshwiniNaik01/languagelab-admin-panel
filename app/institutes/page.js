...
export default function InstitutesPage() {
  ...
  const handleToggleActive = async (id, currentStatus) => {
    ...
    try {
      ...
      const updated = institutes.map(c => c._id === id ? { ...c, is_active: updatedStatus } : c);
...
const [endDate, setEndDate] = useState("");
  ...
  const handleEditSubmit = (data) => {
    const updated = institutes.map(c => c._id === editingInstitute._id ? { ...c, ...data } : c);
    setInstitutes(updated);
    localStorage.setItem("lab_institutes", JSON.stringify(updated));
...
const handleGenerateLicense = () => {
  const storedLicenses =
    JSON.parse(localStorage.getItem("lab_licenses")) || [];
  ...
  localStorage.setItem(
    "lab_licenses",
    JSON.stringify([...storedLicenses, newLicense])
  );
      ...
      <div className="space-y-6">
        ...
        {activeTab === "create" ? (
          <div className="bg-[#FFF8F4] p-0.5 rounded-3xl w-full">
            <InstituteForm initialData={prefilledData || {}} onSubmit={handleCreateSubmit} onCancel={() => setActiveTab("manage")} />
          </div>
        ) : activeTab === "edit" && editingInstitute ? (
          <div className="bg-[#FFF8F4] p-0.5 rounded-3xl w-full">
            <InstituteForm initialData={editingInstitute} onSubmit={handleEditSubmit} onCancel={() => { setActiveTab("manage"); setEditingInstitute(null); }} />
          </div>
        ) : (
            ...
            <div className="bg-white border border-orange-500/20 rounded-3xl p-8 max-w-2xl w-full shadow-2xl relative">
              ...
              <div className="grid grid-cols-2 gap-6 text-sm mb-6">
                ...
                <div>
                  ...
                  <p className="font-bold text-[#3C1E0A] mt-1">{viewingInstitute.phone || "N/A"}</p>
                </div>
                <div>
                ...
                <div className="col-span-2">
                  ...
                  <p className="font-bold text-[#3C1E0A] mt-1">{viewingInstitute.address || "No address provided."}</p>
                </div>
              </div>

              <div className="flex justify-end gap-3 border-t border-orange-500/10 pt-4">
                <Button variant="secondary" onClick={() => setViewingInstitute(null)}>
                  Close Details
                </Button>
              </div>
            </div>
