...
export default function LicensesPage() {
  ...
  // Edit seats modal — sets one license's own concurrent-seat count,
  // independent of the other licenses under the same institute.
  const [editingSeatsLicense, setEditingSeatsLicense] = useState(null);
  ...
  // Pulls { licenses, max_students, students_used } out of the API payload,
  // whatever shape it arrived in — used by loadLicenses and every action below
  // that refreshes an institute's entry after suspend/activate/expire/renew.
  const extractLicenseData = (res) => {
    const payload = res.data?.data || res.data || {};
    const list = payload.licenses || [];
    return {
      licenses: Array.isArray(list) ? list : [],
      max_students: payload.max_students,
      students_used: payload.students_used,
    };
  };
  ...
  const loadLicenses = async (instituteId) => {
    ...
    try {
      ...
      setLicenseMap((prev) => ({
        ...prev,
        [instituteId]: extractLicenseData(res),
      }));
  ...
  const handleRenewLicense = async () => {
    ...
    try {
      ...
      const res = await getInstituteLicenses(renewingLicense.institute_id);

      setLicenseMap((prev) => ({
        ...prev,
        [renewingLicense.institute_id]: extractLicenseData(res),
      }));
  ...
  const openEditSeatsModal = (lic) => {
    setEditingSeatsLicense(lic);
    setNewSeats(lic.total_seats ?? 1);
  };

  const handleUpdateSeats = async () => {
    if (!newSeats || newSeats < 1) {
      Swal.fire({ icon: "warning", title: "Enter at least 1 seat" });
      return;
    }

    setSeatsLoading(true);
    try {
      await updateLicenseSeats(editingSeatsLicense._id, newSeats);

      const res = await getInstituteLicenses(editingSeatsLicense.institute_id);
      setLicenseMap((prev) => ({
        ...prev,
        [editingSeatsLicense.institute_id]: extractLicenseData(res),
      }));

      setEditingSeatsLicense(null);
        ...
        <div className="space-y-6">
          <div>
            ...
            <p className="text-xs font-semibold text-slate-500 mt-1">
              HMAC-signed keys generated per institute. Click an institute to
              expand its keys. Licenses are generated from the Institutes page.
            </p>
          </div>
          ...
          {!loading && institutes.length === 0 && (
            <div className="text-center py-16 text-slate-400 text-sm font-semibold">
              No institutes found. Create institutes first, then generate
              license keys.
            </div>
          )}
          ...
          <div className="space-y-3">
            {institutes.map((inst) => {
              ...
              const capacity = licenseMap[inst._id];
              const licenses = capacity?.licenses || [];
                ...
                >
                  ...
                  >
                    ...
                    <div className="flex items-center gap-6 text-xs text-slate-500 font-semibold">
                      ...
                      <span
                        className={`text-lg font-black text-orange-400 transition-transform ${isOpen ? "rotate-180" : ""}`}
                      >
                        ▼
                      </span>
                    </div>
                  ...
                  {isOpen && (
                    <div className="border-t border-orange-500/10 bg-orange-50/30 px-6 pt-3 pb-4">
                      {typeof capacity?.students_used === "number" && (
                        <div className="flex items-center justify-between mb-3 px-3 py-2 rounded-xl bg-white border border-orange-500/10">
                          <span className="text-[10px] font-black text-slate-500 uppercase tracking-wider">
                            Student seats used
                          </span>
                          <span className="text-xs font-black text-slate-800">
                            {capacity.students_used} / {capacity.max_students}
                          </span>
                        </div>
                      )}
                      {licenses.length === 0 ? (
                        <p className="text-sm text-slate-400 font-semibold text-center py-4">
                          No license keys generated yet. Use the Institutes page
                          to generate keys.
                        </p>
                      ) : (
                        <>
                  ...
                  : "border-l-green-500 border-orange-500/10 bg-white"
                              ...
                              >
                                ...
                                <div>
                                  ...
                                  <span
                                    className={`ml-1 px-1.5 py-0.5 rounded-full border text-[10px] font-bold ${statusColor(lic.status)}`}
                                  >
                                    {lic.status}
                                  </span>
                                </div>
                                ...
                                <div>
                                  ...
                                  <div
                                    className={`text-[10px] font-bold ${daysLeft(lic.expiry_date) <= 30 ? "text-red-600" : "text-orange-600"}`}
                                  >
                                ...
                                <div className="flex items-center gap-1.5 flex-nowrap">
                                  ...
                                  <button
                                    onClick={() => openEditSeatsModal(lic)}
                                    title="Edit this license's seat count"
                                    className="flex items-center gap-0.5 px-2 py-1 text-[10px] font-black rounded-lg bg-purple-50 text-purple-700 border border-purple-200 hover:bg-purple-500 hover:text-white hover:border-purple-500 transition-all whitespace-nowrap"
                                  >
      ...
      {viewingLicense &&
        (() => {
          ...
          const durationPct = Math.min(
            100,
            Math.max(0, ((lic.duration - days) / (lic.duration || 365)) * 100),
          );
              ...
              <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden">
                ...
                <div className="flex items-center justify-between px-6 py-4 bg-gradient-to-r from-orange-500 to-amber-500">
                  ...
                  <div className="flex items-center gap-2">
                    <span
                      className={`px-2.5 py-1 rounded-full text-[10px] font-black border ${statusColor(lic.status)}`}
                    >
                      {lic.status}
                    </span>
                    {lic.is_valid && (
                      <span className="px-2.5 py-1 rounded-full bg-green-100 text-green-700 border border-green-300 text-[10px] font-black">
                        valid
                      </span>
                    )}
                    <button
                      onClick={() => setViewingLicense(null)}
                      className="ml-2 w-7 h-7 rounded-lg bg-white/20 hover:bg-white/30 text-white flex items-center justify-center font-black text-sm cursor-pointer"
                    >
                      ✕
                    </button>
                  </div>
                ...
                <div className="px-6 py-5 space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="bg-orange-50 border border-orange-200 rounded-xl px-4 py-3">
                      ...
                      <p className="font-mono font-black text-orange-700 text-sm break-all">
                        {lic.user_id || "—"}
                      </p>
                    </div>
                    <div className="bg-orange-50 border border-orange-200 rounded-xl px-4 py-3">
                  ...
                  <div>
                    <div className="flex justify-between mb-1.5">
                      ...
                      <span
                        className={`text-[10px] font-black ${days <= 30 ? "text-red-600" : days <= 90 ? "text-amber-600" : "text-green-600"}`}
                      >
                        {days} days left
                      </span>
                    </div>
                  ...
                  <div>
                    <div className="flex justify-between mb-1.5">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span
                          className={`text-[9px] font-black px-2 py-0.5 rounded-full ${lic.has_free_seat ? "bg-green-50 text-green-700 border border-green-200" : "bg-red-50 text-red-600 border border-red-200"}`}
                        >
                  ...
                  <div className="grid grid-cols-2 gap-3 text-xs">
                    ...
                    <div className="bg-slate-50 rounded-xl px-3 py-2.5">
                      ...
                      <p className="font-bold text-slate-700 mt-1">
                        {new Date(lic.updatedAt).toLocaleDateString()}
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="px-6 pb-5">
                  <button
                    onClick={() => setViewingLicense(null)}
                    className="w-full py-2.5 rounded-xl bg-slate-100 text-slate-700 font-bold text-sm hover:bg-slate-200 transition-colors cursor-pointer"
                  >
                    Close
                  </button>
                </div>
          ...
          <div className="bg-white w-full max-w-sm rounded-2xl shadow-xl p-6">
            <div className="flex items-center gap-3 mb-1">
              <div className="w-9 h-9 rounded-xl bg-purple-50 border border-purple-200 flex items-center justify-center shrink-0">
                <Pencil size={16} className="text-purple-600" strokeWidth={2.5} />
              </div>
              <h3 className="text-lg font-black text-slate-950">
                Edit Seat Count
              </h3>
            </div>
            <p className="text-xs text-purple-700 font-mono font-bold mb-5 ml-12">
              {editingSeatsLicense.license_code}
            </p>

            <div>
              <label className="block text-sm font-semibold mb-2">
                Concurrent Students For This License
              </label>
              <div className="flex items-center gap-3">
                <button
                  type="button"
                  className="w-10 h-10 rounded-xl border border-purple-200 font-bold text-lg hover:bg-purple-50 transition"
                  onClick={() => setNewSeats((v) => Math.max(1, v - 1))}
                >
                  −
                </button>
                <input
                  ...
                  className="flex-1 border border-purple-200 rounded-xl px-3 py-2 text-center font-bold"
                />
                <button
                  type="button"
                  className="w-10 h-10 rounded-xl bg-purple-500 text-white font-bold text-lg hover:bg-purple-600 transition"
                  onClick={() => setNewSeats((v) => v + 1)}
                >
                  +
                </button>
              </div>
              <p className="text-xs text-slate-500 mt-2">
                Currently {editingSeatsLicense.active_sessions ?? 0} active
                session{(editingSeatsLicense.active_sessions ?? 0) === 1 ? "" : "s"} on
                this license. Seats can&apos;t be set below that number.
              </p>
            </div>
            ...
            <div className="flex justify-end gap-3 mt-6">
              <Button
                variant="secondary"
                onClick={() => setEditingSeatsLicense(null)}
              >
                Cancel
              </Button>
              <button
                ...
                className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-purple-500 text-white text-sm font-bold hover:bg-purple-600 transition-colors disabled:opacity-50"
              >
                <Pencil size={14} strokeWidth={2.5} />
                {seatsLoading ? "Saving…" : "Save Seats"}
              </button>
            </div>
          </div>
        </div>
