...
export default function LicensesPage() {
    ...
    const handleRevoke = (id) => {
        const updated = licenses.map((l) =>
            l._id === id ? { ...l, status: l.status === 'active' ? 'revoked' : 'active' } : l,
        );
        setLicenses(updated);
    ...
    const columns = [
        {
            ...
            accessor: (row) => (
                <div>
                    <div className="font-mono text-xs text-slate-800 font-bold max-w-xs truncate">
                        {row.license_key}
                    </div>
      ...
      accessor: (row) => (
    ...
    },*/
        {
            ...
            accessor: (row) => {
                const remaining = Math.max(
                    0,
                    Math.ceil((new Date(row.expiry_date) - new Date()) / 86400000),
                );
                return (
            ...
            accessor: (row) => (
                <div className="text-[10px] text-slate-500 font-medium">
                    ...
                    <div className="text-orange-500 font-bold">
                        Latency: {row.llm_metadata?.time_ms || 0}ms
                    </div>
                </div>
            ),
        },
        ...
        },
        {
            ...
            accessor: (row) => (
                <span className="font-bold text-orange-600">{row.license_count || 0}</span>
            ),
        },
        {
            header: 'Actions',
                ...
                <div className="flex justify-between items-center border-b border-orange-200 pb-4">
                    <div>
                        ...
                        <p className="text-xs font-semibold text-slate-500">
                            HMAC-signed keys generated dynamically for institutes. Controls active
                            student seats and license duration.
                        </p>
                    </div>
